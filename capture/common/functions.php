...
function create_error_logs() {
    ...
    $sql = 'create table if not exists tcat_error_ratelimit ( id bigint auto_increment, type varchar(32), start datetime not null, end datetime not null, tweets bigint not null, primary key(id), index(type), index(start), index(end) ) ENGINE=' . MYSQL_ENGINE;
    $h = $dbh->prepare($sql);
    $h->execute();

    $sql = 'create table if not exists tcat_error_gap ( id bigint auto_increment, type varchar(32), start datetime not null, end datetime not null, primary key(id), index(type), index(start), index(end) ) ENGINE=' . MYSQL_ENGINE;
    $h = $dbh->prepare($sql);
    $h->execute();
...
function create_admin() {
    ...
    primary key(id) ) ENGINE = " . MYSQL_ENGINE . " DEFAULT CHARSET = utf8mb4";
    $create = $dbh->prepare($sql);
    $create->execute();
    ...
    KEY `created_at` (`created_at`) ) ENGINE = " . MYSQL_ENGINE . " DEFAULT CHARSET = utf8mb4";
    $create = $dbh->prepare($sql);
    $create->execute();
