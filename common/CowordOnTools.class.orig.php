...
//    	print "<pre>".print_r($this->series,1)."</pre>"; die;
        ...
        foreach($ap as $word => $times) {
			for($i=1;$i<count($timeslices);$i++) {
				$im1 = $i-1;
				if(array_key_exists($timeslices[$im1],$times)===false) continue;
				if(array_key_exists($timeslices[$i],$times)===false) continue;
				$v1 = $times[$im1];
				$v2 = $times[$i];
		...
		foreach($ap as $word => $times) {
			foreach($times as $time => $profile) {
				...
				$prof = "";
				foreach($profile as $w => $c) $prof .= "$w ($c), ";
				$prof = substr($prof,0,-2);
				if(isset($degree[$word][$time])) $deg = $degree[$word][$time]; else $deg = "";
				if(isset($cos_sim[$word][$time])) $cs = $cos_sim[$word][$time]; else $cs = "";
