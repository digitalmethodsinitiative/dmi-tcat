...
//die;
		...
		if($interval == "days") $group = time_to_day($time);
		elseif($interval == "hours") $group = time_to_hour($time);
		elseif($interval == "months") $group = time_to_month($time);
	...
	} else die('no valid output format found');
...
// returns a GEXF file
function toGephi($fromTo,$tags,$title) {
	...
	$gexf = '<?xml version="1.0" encoding="UTF-8"?>
      ...
      version="1.2">
  <meta>
    <creator>tools.digitalmethods.net</creator>
  ...
  <graph defaultedgetype="undirected" mode="static">
	...
	// fromTo = from with array of to's
	...
	foreach($tags as $tag) {
		$tag = xml_escape($tag);
		$tid = "u".md5($tag);
		$nodes .=  "<node id='$tid' label='$tag' />\n";
	}

	foreach($fromTo as $from => $tos) {
		foreach($tos as $to => $c) {
			if(empty($from) || empty($to)) continue; // TODO: log
			$from = xml_escape($from);
			$to = xml_escape($to);
			$eid = "u".md5($from)."u".md5($to);
			$edges .= "<edge id='$eid' source='u".md5($from)."' target='u".md5($to)."' weight='$c' label='$c'/>\n";
		}
	}
	
	$gexf = str_replace("!!!NODES!!!",$nodes,str_replace("!!!EDGES!!!",$edges,$gexf));
//	header("content-type: text/xml"); 
//	header('Content-Disposition: attachment; filename="'.$title.'.gexf"');
//	print $gexf;
	return $gexf;
}
...
function db_connect($db_host,$db_user,$db_pass,$db_name) {
    ...
    if(!mysql_select_db($db_name,$connection)) die("could not connect");
}
