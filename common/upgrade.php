            ...
            if ($ans == 'y' || $ans == 'a') {
                ...
                if (import_mysql_timezone_data() == FALSE) {
                    ...
                    if ($retval != 0) {
                        ...
                        $sql = "create temporary table if not exists tcat_error_ratelimit_upgrade ( id bigint, `type` varchar(32), start datetime not null, end datetime not null, tweets bigint not null, primary key(id, type), index(type), index(start), index(end) ) ENGINE=TokuDB";
                        $rec = $dbh->prepare($sql);
                        $rec->execute();
