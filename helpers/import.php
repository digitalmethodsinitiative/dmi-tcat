...
$hot_conversion = "sed -e 's/^  FULLTEXT KEY `from_user_description` (`from_user_description`),/  KEY `from_user_description` (`from_user_description`(32)),/g' | sed -e 's/^  FULLTEXT KEY `text` (`text`)/  KEY `text` (`text`(32))/g' | sed -e 's/^) ENGINE=MyISAM /) ENGINE=TokuDB COMPRESSION=TOKUDB_LZMA /g' | sed -e 's/DEFAULT CHARSET=utf8;$/DEFAULT CHARSET=utf8mb4;/g'";
